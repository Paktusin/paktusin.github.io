...
export const Projects = () => {
  ...
  const getContent = React.useCallback((project, index) => {
    const res = [
      project.logo && (
        <div
          className={"col-md-6 col-12 d-none d-md-flex position-relative"}
          style={{ minHeight: "100%" }}
        >
          <div
      ...
      <div
        className={clsx({ "col-md-6": project.logo }, "col-12", classes.text)}
      >
        <h3>{project.title}</h3>
        <p>{project.descr}</p>
        {project.url && (
        ...
        {projects.map((project, index) => (
          <div className={"row"} key={index}>
            <div data-scroll={true} className={clsx("col-12 col-12 p-3")}>
              <Card gradient={false} className={clsx(classes.project, "row")}>
